...
def splitter(determinedSymbols):
	for multiplicandLength in range(1,len(determinedSymbols)-1):
		for multiplierLength in range(1,len(determinedSymbols)-multiplicandLength):
			productLength = len(determinedSymbols) - multiplicandLength - multiplierLength
			multiplicand = digitsToNum(determinedSymbols[:multiplicandLength])
			multiplier = digitsToNum(determinedSymbols[multiplicandLength:multiplicandLength+multiplierLength])
			product = digitsToNum(determinedSymbols[multiplicandLength+multiplierLength:])
